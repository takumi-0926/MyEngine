...
//bool Sprite::Createpipeline(ID3D12Device* _dev, SIZE ret)
//{
//	HRESULT result = S_FALSE;
//	ComPtr<ID3DBlob> vsBlob; // 頂点シェーダオブジェクト
//	ComPtr<ID3DBlob> psBlob;	// ピクセルシェーダオブジェクト
...
//		rootSigBlob->GetBufferPointer(),
//		rootSigBlob->GetBufferSize(),
//		IID_PPV_ARGS(_pipelineSet._rootsignature.ReleaseAndGetAddressOf()));
//	if (FAILED(result)) {
//		assert(0);
//		return false;
//	}
//
//	パイプラインにルートシグネチャをセット
//	gpipeline.pRootSignature = _pipelineSet._rootsignature.Get();
//
//	 グラフィックスパイプラインの生成
//	result = device->CreateGraphicsPipelineState(
//		&gpipeline, IID_PPV_ARGS(_pipelineSet._pipelinestate.ReleaseAndGetAddressOf()));
//	if (FAILED(result)) {
//		assert(0);
//		return false;
//	}
...
//	descHeapDesc.NumDescriptors = spritecommon.spriteSRVCount;
//	result = device->CreateDescriptorHeap(
//		&descHeapDesc, IID_PPV_ARGS(&spritecommon._descHeap));//生成
//	if (FAILED(result)) {
//		assert(0);
//		return false;
//	}
